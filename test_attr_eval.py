...
DEFAULT_LABELS_FILE = Path("labels_full_template.csv")
DEFAULT_REPORT_FILE = Path("test_attr_eval_report.json")
...
def build_quality_bucket_summary(items: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "color_hit_rate": safe_ratio(sum(1 for item in color_rows if item["color_eval"]["hit_count"] > 0), len(color_rows)),
        "occasion_hit_rate": safe_ratio(sum(1 for item in occasion_rows if item["occasion_eval"]["hit_count"] > 0), len(occasion_rows)),
        "season_hit_rate": safe_ratio(sum(1 for item in season_rows if item["season_eval"]["hit_count"] > 0), len(season_rows)),
        "counts": {
...
def summarize_quality_results(results: list[dict[str, Any]], failures_limit: int, topk: int) -> dict[str, Any]:
    ...
    failed_items = {
        ...
        "occasion": [
            ...
            if item["occasion_eval"]["hit_count"] == 0
        ][:failures_limit],
        "season": [
            {
                "file": item["file"],
            ...
            if item["season_eval"]["hit_count"] == 0
        ][:failures_limit],
